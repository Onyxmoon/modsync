...
public final class PermissionHelper {
    ...
    /**
     * Check if the command sender is allowed to use ModSync commands.
     * Console is always allowed; players require the modsync.admin permission.
     *
     * @param commandContext the command context
     * @return true if allowed, false otherwise
     */
    public static boolean checkAdminPermission(CommandContext commandContext) {
        if (!commandContext.isPlayer()) {
            return true;
        }
        if (!hasAdminPermission(commandContext.sender().getUuid())) {
            commandContext.sendMessage(Message.raw("You don't have permission to use this command.").color(Color.RED));
            return false;
        }
        return true;
    }
