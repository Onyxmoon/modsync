...
public class RemoveCommand extends CommandBase {
    ...
    protected void executeSync(@Nonnull CommandContext commandContext) {
        ...
        if (registry.isEmpty()) {
            sender.sendMessage(Message.raw("No mods in list.").color(Color.YELLOW));
            return;
        }
        ...
        handleSelectionResult(sender, result);
    }

    private void handleSelectionResult(CommandSender sender, SelectionResult result) {
        switch (result) {
            case SelectionResult.Found found -> removeMod(sender, found.mod());
            ...
            case SelectionResult.InvalidIndex invalid ->
                sender.sendMessage(Message.raw("Use name, slug, or identifier to remove mods.").color(Color.RED));
    ...
    private void showHelp(CommandSender sender, ManagedModRegistry registry) {
        ...
        sender.sendMessage(Message.raw("=== Managed Mods (" + mods.size() + ") ===").color(Color.CYAN));

        for (ManagedMod mod : mods) {
            sender.sendMessage(CommandUtils.formatModLineWithStatus(mod));
        }
    }

    private void removeAllMods(CommandSender sender, ManagedModRegistry registry) {
        List<ManagedMod> mods = registry.getAll();
        int total = mods.size();

        sender.sendMessage(Message.raw("Removing " + total + " mod(s)...").color(Color.YELLOW));
        ...
        CompletableFuture.allOf(futures)
                .thenRun(() -> {
                    sender.sendMessage(Message.raw("Removed: " + removed.get()).color(Color.GREEN));
                    sender.sendMessage(Message.raw("Removed: " + removed.get()).color(Color.GREEN)
                            .insert(Message.raw(" | Failed: " + failed.get()).color(failed.get() > 0 ? Color.RED : Color.GRAY)));
                    if (pendingRestart.get() > 0) {
                        sender.sendMessage(Message.raw(pendingRestart.get() + " file(s) locked. Restart server to complete deletion.").color(Color.YELLOW));
                    }
                    if (removed.get() > 0) {
                        sender.sendMessage(Message.raw("Server restart required to fully unload mods.").color(Color.CYAN));
                    }
                });
    }

    private void removeMod(CommandSender sender, ManagedMod mod) {
        if (mod.isInstalled()) {
            sender.sendMessage(Message.raw("Removing installed file...").color(Color.YELLOW));

            modSync.getDownloadService().deleteMod(mod)
                    .thenAccept(deletedImmediately -> {
                        modSync.getManagedModStorage().removeMod(mod.getSourceId());
                        if (deletedImmediately) {
                            sender.sendMessage(Message.raw("Removed: ").insert(CommandUtils.formatModLine(mod)).insert(" (file deleted)").color(Color.GREEN));
                            sender.sendMessage(Message.raw("Server restart required to fully unload the mod.").color(Color.CYAN));
                        } else {
                            sender.sendMessage(Message.raw("Removed: ").insert(CommandUtils.formatModLine(mod)).insert(" (file deleted)").color(Color.GREEN));
                            sender.sendMessage(Message.raw("File is locked. Restart server to complete deletion.").color(Color.YELLOW));
                        }
                    })
                    .exceptionally(ex -> {
                        sender.sendMessage(Message.raw("Failed to delete file: " + ex.getMessage()).color(Color.RED));
                        modSync.getManagedModStorage().removeMod(mod.getSourceId());
                        sender.sendMessage(Message.raw("Removed from list (file may remain)").color(Color.YELLOW));
                        return null;
                    });
        } else {
            modSync.getManagedModStorage().removeMod(mod.getSourceId());
            sender.sendMessage(Message.raw("Removed: ").insert(CommandUtils.formatModLine(mod)).color(Color.GREEN));
        }
    }
